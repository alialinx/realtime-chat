...
# region GROUPS
@router.get("/groups", summary="get my groups", tags=["Groups"])
def get_groups(current: dict = Depends(current_user)):
    con, cur = get_db()
    ...
    try:
        ...
        return {"success": True, "message": "all group", "data": groups}
...
def get_group(group_id: int, current: dict = Depends(current_user)):
    ...
    try:
        ...
        cur.execute("SELECT * FROM groups WHERE owner_id = %s AND id = %s", (user_id, group_id))
...
def update_group(payload: UpdateGroup, group_id: int, current: dict = Depends(current_user)):
    ...
    try:
        ...
        current_name, current_desc = group[0], group[1]
...
@router.put("/groups/{group_id}/members/{member_id}/role", summary="  Update  member role in groups", tags=["Group Members"])
...
def get_group_messages(page:int, limit:int,group_id: int, current: dict = Depends(current_user)):
    ...
    try:
        ...
        cur.execute("SELECT * FROM group_messages WHERE group_id = %s ORDER BY created_at DESC LIMIT %s, OFFSET %s ", (group_id,limit,page))
